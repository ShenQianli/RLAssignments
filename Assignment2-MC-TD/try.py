...
class RandomWalk(object):
    ...
    def step(self):
        """
        Randomly choose a direction for a step (Left or Right).
        Return the new state and the reward.
        """
        reward = 0
        # TODO: Compute the next state and the corresponding reward by filling in your code here
        # begin your code

        # end your code
        return self._state, reward

# TODO: try to manually calculate the true values
...
def mc(states_col, rewards_col, alpha=.03, plot=False):
    ...
    for i, states, rewards in zip(np.arange(len(states_col)), states_col, rewards_col):
        ...
        # TODO: For each state in the episode, perform MC update for `preds`
        # begin your code
...
def td(states_col, rewards_col, alpha=.03, plot=False):
    ...
    for i, states, rewards in zip(np.arange(len(states_col)), states_col, rewards_col):
        if i in [0, 1, 10, 100]:
            if plot:
                plt.plot(np.arange(5), preds, label='n={}'.format(i))
        # TODO: For each state in the episode, perform TD(0) update for `preds`
        # begin your code
